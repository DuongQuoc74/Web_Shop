...
namespace eShpsolution.Application.Catalog.Products
{
    ...
    {
        private readonly EShopDbContext _context;//khái báo biến cục bộ chỉ đọc 1 lần
        private readonly IStorageService _storageService;
        private const string USER_CONTENT_FOLDER_NAME = "user_content";
        public ProductService(EShopDbContext context, IStorageService storageService)// contrucstor
        {
            _context = context;
        ...
        public async Task<PagedResult<ProductVm>> GetAllPaging(GetManageProductPadingRequest request)
        {
            //1 select leff join
        ...
        public async Task<int> Update(ProductUpdateRequest request)
        {
            ...
            if (product == null || productTranslation==null) throw new EShopException($"Can'n find a product with id:{request.Id} ");
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new EShopException($"can'n find a product with id: {productId}");
            product.Price = newPrice;

            return await _context.SaveChangesAsync() > 0;
        }

        public async Task<bool> UpdateStock(int productId, decimal AddedQuantity)
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new EShopException($"can'n find a product with id: {productId}");
        ...
        public async Task<ProductImageViewModel> GetImageById(int imageId)
        {
            ...
            if (image == null)
                throw new EShopException($"Can't find an Image withid {imageId}");
        ...
        public async Task<PagedResult<ProductVm>> GetAllByCategoryId(string languageId, GetPublicProductPadingRequest request)
        {
            //1, select
            var query = from p in _context.Products
                        join pt in _context.ProductTranslations on p.Id equals pt.ProductId
		...
		public async Task<List<ProductVm>> GetFeaturedProducts(string languageId, int take)
		{
			var query = from p in _context.Products
                        ...
                        from c in picc.DefaultIfEmpty()
                        where pt.LanguageId== languageId
						&& (pi==null || pi.IsDefault==true) && p.IsFeatured == true
                        select new {p,pt, pic, pi};
